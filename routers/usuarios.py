...
def create_usuario(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    db_usuario = db.query(Usuario).filter(Usuario.matricula == usuario.matricula).first()
    if db_usuario:
...
def add_deporte_to_usuario(
    ...
    if deporte in usuario.deportes:
        raise HTTPException(status_code=400, detail="El usuario ya tiene este deporte")
...
def remove_deporte_from_usuario(
    ...
    if deporte not in usuario.deportes:
        raise HTTPException(status_code=400, detail="El usuario no tiene este deporte")
